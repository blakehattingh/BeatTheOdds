...
def CVaRModel(Pk, odds, betsConsidered, alphas, betas):
    # Inputs:
    ...
    # - alphas and betas are parameters corresponding to the users risk profile
    ...
    for b in betas:
        Problem += (1. / b) * lpSum([Pk[k] * ((1 - b) * wkMatrix[b][k] + b * vkMatrix[b][k]) for k in Pk.keys()]) <= alphas[b] * lpSum([oddCoef[bet] * Bets[bet] - Bets[bet] for bet in oddCoef.keys()]) #risks[b]
